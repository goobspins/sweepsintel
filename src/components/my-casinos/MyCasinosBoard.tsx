  ...
  return (
    <div className="portfolio-shell">
      ...
      <section className="surface-card board-card">
        ...
        <div className="card-list">
          {sortedCasinos.map((casino) => {
            ...
            const healthDetail = healthByCasino[casino.casino_id];
              ...
              <article key={casino.casino_id} className={`casino-card health-${casino.health_status ?? 'unknown'}`} id={`casino-${casino.casino_id}`}>
                <button type="button" className="casino-card-head" onClick={() => void toggleExpanded(casino.casino_id)}>
                  <div className="health-column">
                    <HealthDot status={casino.health_status} size={18} pulse={casino.health_status === 'critical'} />
                  </div>
                  <div className="casino-main">
                    <div className="casino-title-row">
                      ...
                      {casino.alert_count > 0 ? <span className="alert-pill">{casino.alert_count} alerts</span> : null}
                    </div>
                    <div className="casino-subline">
                      <span className="metric-cluster"><span className="metric-label">SC Balance</span><strong>{formatSc(casino.sc_balance)}</strong></span>
                      <span className="metric-cluster"><span className="metric-label">Net P/L</span><strong className={casino.net_pl_usd >= 0 ? 'positive' : 'negative'}>{formatCurrency(casino.net_pl_usd)}</strong></span>
                      <span className="metric-cluster"><span className="metric-label">Last Activity</span><strong>{casino.last_activity_at ? formatDateTime(casino.last_activity_at) : 'No entries yet'}</strong></span>
                    </div>
                    {casino.health_reason ? <p className="muted reason-line">{casino.health_reason}</p> : null}
                  </div>
                </button>
                ...
                {expanded ? (
                  <div className="expanded-panel">
                    <section className="expanded-section">
                      <h2 className="expanded-title">Financial Summary</h2>
                      <div className="financial-grid">
                        <div className="summary-chip"><span className="metric-label">SC Balance</span><strong>{formatSc(casino.sc_balance)}</strong></div>
                        <div className="summary-chip"><span className="metric-label">USD Value</span><strong>{formatCurrency(casino.usd_value)}</strong></div>
                        <div className="summary-chip"><span className="metric-label">Invested</span><strong>{formatCurrency(casino.total_invested_usd)}</strong></div>
                        <div className="summary-chip"><span className="metric-label">Redeemed</span><strong>{formatCurrency(casino.total_redeemed_usd)}</strong></div>
                      </div>
                      <div className="entry-list">
                        {casino.recent_entries.slice(0, 10).map((entry) => (
                      ...
                      <a href={`/ledger?casino_id=${casino.casino_id}`} className="subtle-link">View in Ledger</a>
                    </section>

                    <section className="expanded-section">
                      <h2 className="expanded-title">Health Detail</h2>
                      {loadingHealthId === casino.casino_id ? (
                        <p className="muted">Loading health detail...</p>
                      ) : healthDetail ? (
                        <HealthDetail detail={healthDetail} />
                      ) : (
                    ...
                    <section className="expanded-section">
                      <h2 className="expanded-title">Casino Quick Reference</h2>
                      <div className="reference-grid">
                        <div className="summary-chip"><span className="metric-label">Tier</span><strong>{casino.tier ?? '--'}</strong></div>
                        <div className="summary-chip"><span className="metric-label">PB Risk</span><strong>{casino.promoban_risk ?? '--'}</strong></div>
                        <div className="summary-chip"><span className="metric-label">Redeem Speed</span><strong>{casino.redemption_speed_desc ?? '--'}</strong></div>
                        <div className="summary-chip"><span className="metric-label">Daily Bonus</span><strong>{casino.daily_bonus_desc ?? '--'}</strong></div>
                      </div>
                      <label className="notes-label" htmlFor={`notes-${casino.casino_id}`}>Personal notes</label>
                      <textarea
                      ...
                      />
                      <span className="muted notes-hint">{savingNotesId === casino.casino_id ? 'Saving...' : 'Saves on blur'}</span>
                    </section>

                    <section className="expanded-section">
                      <h2 className="expanded-title">Quick Actions</h2>
                      <div className="quick-actions">
                        <a href={`/dashboard#casino-${casino.casino_id}`} className="action-link action-primary">Claim Daily</a>
                        <a href={`/dashboard#casino-${casino.casino_id}`} className="action-link">Log Purchase</a>
                        <a href="/redemptions" className="action-link">Submit Redemption</a>
                        {casino.visit_url ? <a href={casino.visit_url} className="action-link" target="_blank" rel="noopener noreferrer">Visit Casino</a> : null}
                        <a href={`/casinos/${casino.slug}`} className="action-link action-secondary">View Profile</a>
                      </div>
                    </section>
                  </div>
                ) : null}
      ...
      <style>{`
        ...
        .card-list { display:grid; gap:.9rem; }
        .casino-card { border:1px solid var(--color-border); border-radius:1.35rem; background:rgba(17, 24, 39, 0.54); overflow:hidden; }
        .casino-card-head { width:100%; border:none; background:transparent; color:inherit; padding:1rem 1.05rem; display:grid; grid-template-columns:auto 1fr; gap:1rem; cursor:pointer; text-align:left; }
        .health-column { padding-top:.25rem; }
        .casino-main { display:grid; gap:.65rem; }
        .casino-title-row, .casino-subline, .quick-actions { display:flex; gap:.75rem; flex-wrap:wrap; align-items:center; }
        .casino-name { color:var(--text-primary); text-decoration:none; font-size:1.1rem; font-weight:800; letter-spacing:-.03em; }
        .tier-badge, .alert-pill { display:inline-flex; min-width:2rem; justify-content:center; border-radius:999px; padding:.25rem .55rem; font-size:.78rem; font-weight:800; border:1px solid transparent; }
        .alert-pill { background:rgba(239,68,68,.12); color:var(--accent-red); border-color:rgba(239,68,68,.24); }
        .metric-cluster { display:inline-flex; gap:.45rem; align-items:baseline; flex-wrap:wrap; }
        .reason-line { margin:0; }
        .expanded-panel { display:grid; gap:1rem; padding:0 1.05rem 1.05rem; }
        ...
        .expanded-title { margin:0; font-size:1rem; }
        ...
        .action-secondary, .subtle-link { color:var(--text-secondary); }
        ...
        @media (max-width: 960px) { .financial-grid, .reference-grid { grid-template-columns:repeat(2, minmax(0, 1fr)); } .sort-panel { justify-items:start; } .sort-pills { justify-content:flex-start; } }
        @media (max-width: 640px) { .financial-grid, .reference-grid { grid-template-columns:1fr; } .quick-actions, .sort-pills { display:grid; } .action-link, .sort-pill { width:100%; } .entry-row { display:grid; } .entry-values { justify-items:start; text-align:left; } }
